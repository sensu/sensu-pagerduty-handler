...
var (
	...
	pagerDutyConfigOptions = []*sensu.PluginConfigOption{
		...
		},
		{
			...
			Env:      "PAGER_TEAM",
			Argument: "pager-team",
			Usage:    "String for envvar name(alphanumeric and underscores) holding PagerDuty V2 API authentication token, can be set with PAGER_TEAM",
			Value:    &config.teamName,
			Default:  "",
		},
		{
			Path:     "pager-team-suffix",
			Env:      "PAGER_TEAM_SUFFIX",
			Argument: "pager-team-suffix",
			Usage:    "Pager team suffix string to append if missing from pager-team name, can be set with PAGER_TEAM_SUFFIX",
